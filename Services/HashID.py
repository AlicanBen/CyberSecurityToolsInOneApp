...
class HashID:
    logo = "###########################################################################\n"
    logo+="#     __  __                     __           ______    _____         \t\t\t\t\t\t\t\t#\n"
    logo+="#    /\ \/\ \                   /\ \         /\__  _\  /\  _ `\         \t\t\t\t\t\t\t\t#\n"
    logo+="#    \ \ \_\ \     __      ____ \ \ \___     \/_/\ \/  \ \ \/\ \        \t\t\t\t\t\t\t\t#\n"
    logo+="#     \ \  _  \  /'__`\   / ,__\ \ \  _ `\      \ \ \   \ \ \ \ \       \t\t\t\t\t\t\t\t#\n"
    logo+="#      \ \ \ \ \/\ \_\ \_/\__, `\ \ \ \ \ \      \_\ \__ \ \ \_\ \      \t\t\t\t\t\t\t\t#\n"
    logo+="#       \ \_\ \_\ \___ \_\/\____/  \ \_\ \_\     /\_____\ \ \____/      \t\t\t\t\t\t\t\t#\n"
    logo+="#        \/_/\/_/\/__/\/_/\/___/    \/_/\/_/     \/_____/  \/___/  v1.1\t\t\t\t\t\t\t\t#\n"
    logo+="#                                                             By Zion3R\t\t\t\t\t\t\t\t#\n"
    logo+="#                                                    www.Blackploit.com\t\t\t\t\t\t\t\t#\n"
    logo+="#                                                   Root@Blackploit.com\t\t\t\t\t\t\t\t#\n"
    logo += "#######################################################################\n"
    ...
    jerar = []
    # self.hash.islower()  minusculas
    # self.hash.isdigit()  numerico
    # self.hash.isalpha()  letras
    # self.hash.isalnum()  alfanumerico
    hash = ""
    ...
    def CRC16(self):
        hs = '4607'
        if len(self.hash) == len(hs) and self.hash.isalpha() == False and self.hash.isalnum() == True:
            self.jerar.append("101020")

    def CRC16CCITT(self):
        hs = '3d08'
        if len(self.hash) == len(
                hs) and self.hash.isdigit() == False and self.hash.isalpha() == False and self.hash.isalnum() == True:
    ...
    def GHash323(self):
        hs = '80000000'
        if len(self.hash) == len(
                hs) and self.hash.isdigit() == True and self.hash.isalpha() == False and self.hash.isalnum() == True:
    ...
    def DESUnix(self):
        hs = 'ZiY8YtDKXJwYQ'
        if len(self.hash) == len(hs) and self.hash.isdigit() == False and self.hash.isalpha() == False:
    ...
    def DomainCachedCredentials(self):
    ...
    def MD5phpBB3(self):
        ...
        if len(self.hash) == len(
                hs) and self.hash.isdigit() == False and self.hash.isalpha() == False and self.hash.isalnum() == False and self.hash[
                                                                                                                           0:3].find(
            '$H$') == 0:
    ...
    def MD5APR(self):
        ...
        if len(self.hash) == len(hs) and self.hash.isdigit() == False and self.hash.isalpha() == False and self.hash[
                                                                                                           0:4].find(
                '$apr') == 0:
    ...
    def SAM(self):
        ...
        if len(self.hash) == len(
                hs) and self.hash.isdigit() == False and self.hash.isalpha() == False and self.hash.isalnum() == False and self.hash.islower() == False and self.hash[
                                                                                                                                                            32:33].find(
            ':') == 0:
            self.jerar.append("116040")

    def SHA256Django(self):
    ...
    def exec(self):
       ...
       -------------------------------------------------------------------------""")
        ...
        self.MD5APR()
        self.MD5HMAC()
        self.MD5HMACWordpress()
        self.MD5phpBB3()
        self.MD5Unix()
        self.MD5Wordpress()
        self.MD5Half()
        self.MD5Middle()
        self.MD5passsaltjoomla1()
        self.MD5passsaltjoomla2()
        self.MySQL()
        self.MySQL5()
        self.MySQL160bit()
        self.NTLM()
        ...
        self.md5saltmd5pass()
        self.md5saltpass()
        self.md5saltpasssalt()
        self.md5saltpassusername()
        self.md5saltmd5pass()
        self.md5saltmd5passsalt()
        self.md5saltmd5passsalt()
        self.md5saltmd5saltpass()
        self.md5saltmd5md5passsalt()
        self.md5username0pass()
        self.md5usernameLFpass()
        self.md5usernamemd5passsalt()
        self.md5md5pass()
        self.md5md5passsalt()
        self.md5md5passmd5salt()
        self.md5md5saltpass()
        self.md5md5saltmd5pass()
        self.md5md5usernamepasssalt()
        self.md5md5md5pass()
        self.md5md5md5md5pass()
        self.md5md5md5md5md5pass()
        self.md5sha1pass()
        self.md5sha1md5pass()
        self.md5sha1md5sha1pass()
        self.md5strtouppermd5pass()
        self.sha1passsalt()
        self.sha1saltpass()
        self.sha1saltmd5pass()
        self.sha1saltmd5passsalt()
        self.sha1saltsha1pass()
        self.sha1saltsha1saltsha1pass()
        self.sha1usernamepass()
        self.sha1usernamepasssalt()
        self.sha1md5pass()
        self.sha1md5passsalt()
        self.sha1md5sha1pass()
        self.sha1sha1pass()
        self.sha1sha1passsalt()
        self.sha1sha1passsubstrpass03()
        self.sha1sha1saltpass()
        self.sha1sha1sha1pass()
        self.sha1strtolowerusernamepass()


        if len(self.jerar) == 0:

            ret.append( " Not Found.")
        elif len(self.jerar) > 2:
            self.jerar.sort()
